...
- Observable contract, pinned by `crates/sidebar/src/sidebar_tests.rs`: closing
  or archiving the last session on a panel with no already-parked draft leaves
  that panel with **no active view at all** (`active_entry` is `None`), not a
  fresh empty draft. This applies even when the closed/archived item was a real
  thread, not just an empty draft — see
  `test_closing_last_agent_panel_terminal_leaves_empty_header`,
  `test_archive_thread_active_entry_management`, and
  `test_switch_to_workspace_with_archived_thread_shows_no_active_entry`. A
  draft only reappears automatically if one was already parked before the
  close/archive (e.g. via `create_new_thread`) and survived it untouched.
